...
export async function POST(request) {
  try {
    ...
    const pb = new PocketBase("https://admin.gdasconference.com")
    ...
    const record = await pb.collection("GDAS_journal_form_submission").create(pbFormData)
    ...
    await resend.emails.send({
      from: "GDAS |Journal-Submission <info@gdasconference.com>",
      to: data.email,
      subject:
        "Journal Submission Confirmation - GDAS ",
      html: getUserEmailTemplate(data),
    })

    // Send notification email to admin
    await resend.emails.send({
      from: "GDAS | Submission <info@gdasconference.com>",
      to: "info@gdasconference.com", // Replace with actual admin email
      subject: "New Journal Submission - from GDAS",
